...
const Quiz = () => {
  ...
  // 2. Auto-advance or Auto-submit on timeout
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleDisqualification = () => setIsDisqualified(true);
    window.addEventListener("studentDisqualified", handleDisqualification);
    return () => window.removeEventListener("studentDisqualified", handleDisqualification);
  }, [navigate]);
  ...
  const handleAnswerChange = (questionIndex, optionIndex) => {
    ...
    // Auto-advance REMOVED here to wait for "Next" click
  };
  ...
  if (showResults && currentView === "results") {
    ...
    return (
      <QuizResultsScreen
        ...
        answers={answers}
        handleViewCertificate={() => navigate("/certificate")}
